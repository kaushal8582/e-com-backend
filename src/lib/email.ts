...
async function sendViaRelay(mailOptions: {
  ...
  try {
    ...
    if (!res.ok) console.error('[email] relay failed', res.status);
  } catch (err) {
...
export async function sendPasswordReset(toEmail: string, resetLink: string): Promise<void> {
...
export async function sendOrderConfirmation(order: IOrder, userEmail: string): Promise<void> {
  const orderDate =
  ...
  const rows = order.items
    .map(
      (i) =>
        `<tr><td>${i.titleSnapshot}</td><td>${i.qty}</td><td>₹${i.priceSnapshot}</td><td>₹${i.qty * i.priceSnapshot}</td></tr>`
    )
    .join('');
